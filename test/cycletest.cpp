...
#include "cycletest.h"
#include "../source/cycles.h"
#include "../source/graphgeneration.h"
...
void cycletest::runTest3()
{
    ...
    auto g = Grafz::generate_complete_graph(vs);
    auto cyclesv = Grafz::find_all_cycles<>(*(g.get()));
    cout << "Cycle count for complete graph K6: " << cyclesv.size() << endl;
    CPPUNIT_ASSERT(cyclesv.size() == 197); //see: http://mathworld.wolfram.com/CompleteGraph.html
}



void cycletest::runTest4()
{
    vector<std::shared_ptr<base_vertex>> vs;
    vector<std::shared_ptr<weighted_edge<base_vertex>>> wes;
    ...
    auto nodes = Grafz::generate_vertices(5);
    auto g = Grafz::generate_cyclic_graph(vs);
    auto cyclesv = Grafz::find_all_cycles<>(*(g.get()));
    auto func = [](base_vertex& t){auto bv = make_shared<base_vertex>(t.get_label()); return bv;};
    
    std::function<shared_ptr<base_vertex>(base_vertex&)> f = func;
    auto weightedg = Grafz::generate_random_weights(g, f);
    cout << "Cycle count for cyclic graph C5: " << cyclesv.size() << endl;
    CPPUNIT_ASSERT(cyclesv.size() == 1);
}
